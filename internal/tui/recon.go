...
func (s Severity) String() string {
	switch s {
	case SevCritical: return "CRIT"
	case SevHigh:     return "HIGH"
	case SevMedium:   return "MED"
...
func NewReconModel() *ReconModel {
	...
	s.Header = s.Header.
		BorderStyle(lipgloss.NormalBorderClr).
...
func (m *ReconModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		switch msg.String() {
		case "1": m.filterLevel = SevCritical; m.applyFilter()
		case "2": m.filterLevel = SevHigh;     m.applyFilter()
		case "3": m.filterLevel = SevMedium;   m.applyFilter()
		case "4": m.filterLevel = SevLow;      m.applyFilter()
		case "5": m.filterLevel = SevInfo;     m.applyFilter()
		case "0": m.filterLevel = 0;           m.applyFilter()
		}
	}
	var cmd tea.Cmd
...
func (m *ReconModel) View() tea.View {
	...
	filter := lipgloss.NewStyle().Foreground(MutedClr).Render("[1]CRIT [2]HIGH [3]MED [4]LOW [5]INFO [0]ALL")
